...
const Cocina = () => {
  ...
  useEffect(() => {
    const fetchPedidos = async () => {
      try {
        const response = await fetch('https://fullstack-liart-six.vercel.app/restaurante/pedidos');
        if (!response.ok) {
  ...
  const handleMarcarComoListo = async (id) => {
    try {
        const response = await fetch(`https://fullstack-liart-six.vercel.app/restaurante/pedidos/${id}`, {
            method: 'PATCH',
            headers: {
        ...
        const ventaResponse = await fetch('https://fullstack-liart-six.vercel.app/restaurante/ventas', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
